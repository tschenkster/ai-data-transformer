-- Add unique constraints to support ON CONFLICT in grant_entity_access function

-- First, add constraint for entity-specific access (when entity_uuid is not null)
ALTER TABLE user_entity_access 
ADD CONSTRAINT unique_user_entity_access_active 
UNIQUE (user_account_uuid, entity_uuid) 
WHERE is_active = true AND entity_uuid IS NOT NULL;

-- Add constraint for entity group access (when entity_group_uuid is not null)
ALTER TABLE user_entity_access 
ADD CONSTRAINT unique_user_entity_group_access_active 
UNIQUE (user_account_uuid, entity_group_uuid) 
WHERE is_active = true AND entity_group_uuid IS NOT NULL;

-- Also update the grant_entity_access function to handle entity group access properly
