...
-- Step 1: Drop existing primary key constraint (checking current constraint name)
DO $$ 
BEGIN
    -- Drop the primary key constraint if it exists
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'report_line_items_pkey' 
        AND table_name = 'report_line_items'
    ) THEN
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'report_structures_pkey' 
        AND table_name = 'report_structures'
    ) THEN
...
-- Step 1: Update account_mappings to reference UUID instead of integer
...
-- Step 2: Update report_line_items to reference report_structure_uuid instead of report_structure_id
-- Add the new UUID column if it doesn't exist
