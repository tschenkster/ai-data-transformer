-- Security hardening - Database-level fixes only
-- Auth configuration must be done via Supabase dashboard

-- 1. Create rate limiting function for sensitive operations
...
-- 2. Enhanced security audit logging function
...
-- 3. Add security monitoring for failed operations
...
-- 5. Create security monitoring trigger for user account changes
...
-- Apply the trigger
DROP TRIGGER IF EXISTS audit_user_account_changes_trigger ON public.user_accounts;
CREATE TRIGGER audit_user_account_changes_trigger
