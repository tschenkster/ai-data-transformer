-- Phase 6: Enhanced Authentication & Security Features
...
USING (
    EXISTS (
        ...
        AND EXISTS (
            ...
            JOIN public.roles r ON uar.role_id = r.role_id
            WHERE uar.user_uuid = ua.user_uuid
...
CREATE OR REPLACE FUNCTION public.check_rate_limit(identifier TEXT, action_type TEXT, max_attempts INTEGER DEFAULT 5, window_minutes INTEGER DEFAULT 15)
RETURNS JSONB
SECURITY DEFINER
...
-- 7. Enhanced RLS policies for better security on existing tables
...
-- Restrict audit log access further
DROP POLICY IF EXISTS "Admin users can view all audit logs" ON public.security_audit_logs;
...
CREATE POLICY "Admin can view entity audit logs"
ON public.security_audit_logs FOR SELECT
USING (
    EXISTS (
        SELECT 1 FROM public.user_accounts ua
        ...
        AND EXISTS (
            SELECT 1 FROM public.user_account_roles uar
            JOIN public.roles r ON uar.role_id = r.role_id
            JOIN public.entity_access ea ON ea.user_uuid = ua.user_uuid
            WHERE uar.user_uuid = ua.user_uuid
            AND r.role_name = 'admin'
            AND ea.entity_uuid = security_audit_logs.entity_uuid
        )
    )
);
