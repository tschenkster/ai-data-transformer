...
-- Fix ON CONFLICT predicate to match existing unique partial indexes on user_entity_access
-- Function 1: unified grant (entity or group)
...
-- Function 2: entity-only overload
CREATE OR REPLACE FUNCTION public.grant_entity_access(
  p_user_uuid uuid,
  p_entity_uuid uuid,
  p_access_level access_level,
...
BEGIN
  ...
  -- If no inactive record was updated, insert or upsert on active
  IF NOT FOUND THEN
