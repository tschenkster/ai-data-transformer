-- Fix unique_sort_order_per_structure violations by offsetting first
-- Move any sibling resequencing to after the global offset; ensure temp values are unique across structure
...
CREATE OR REPLACE FUNCTION public.reorder_line_item_with_hierarchy(
  ...
  p_drop_position text DEFAULT NULL,
...
BEGIN
  ...
  -- Optional rename first (does not touch sort_order)
  ...
  -- Parent update (no sort_order changes yet)
  ...
  -- PHASE A: Offset ALL items first to avoid unique collisions
  UPDATE report_line_items 
  SET sort_order = sort_order + 100000
  WHERE report_structure_uuid = p_structure_uuid;

  -- PHASE B: Place moved item among target siblings using small temporary range
  IF p_target_position IS NOT NULL THEN
    IF p_new_parent_uuid IS NULL THEN
      -- root level siblings (exclude moved)
      WITH reseq AS (
      ...
      SET sort_order = 10000 + b.new_rn
      FROM bumped b
      WHERE r.report_line_item_id = b.report_line_item_id;

      UPDATE report_line_items
      SET sort_order = 10000 + p_target_position
      WHERE report_line_item_uuid = p_moved_item_uuid
        AND report_structure_uuid = p_structure_uuid;
    ELSE
      -- non-root
  ...
  -- PHASE C: Recompute hierarchy_path and level_x descriptions for moved subtree
  ...
  -- Optional key regeneration
  IF p_regenerate_keys THEN
    WITH RECURSIVE tree AS (
      SELECT r.report_line_item_uuid, r.parent_report_line_item_uuid, r.hierarchy_path
      FROM report_line_items r
      WHERE r.report_structure_uuid = p_structure_uuid AND r.report_line_item_uuid = p_moved_item_uuid
      UNION ALL
      SELECT c.report_line_item_uuid, c.parent_report_line_item_uuid, c.hierarchy_path
      FROM report_line_items c
    ...
    SET report_line_item_key = (
      WITH gen AS (
        SELECT k.base_key AS candidate FROM keys k WHERE k.report_line_item_uuid = r.report_line_item_uuid
      )
      SELECT COALESCE(
        (SELECT candidate FROM gen WHERE NOT EXISTS (
          SELECT 1 FROM report_line_items r2 WHERE r2.report_structure_uuid = p_structure_uuid AND r2.report_line_item_key = (SELECT candidate FROM gen) AND r2.report_line_item_uuid <> r.report_line_item_uuid
        )),
        (
          SELECT candidate || '_' || i::text
          FROM gen, generate_series(1, 1000) AS g(i)
          WHERE NOT EXISTS (
            SELECT 1 FROM report_line_items r3 WHERE r3.report_structure_uuid = p_structure_uuid AND r3.report_line_item_key = (SELECT candidate FROM gen) || '_' || i::text
          )
          LIMIT 1
        )
      )
    ...
    WHERE r.report_structure_uuid = p_structure_uuid
      AND r.report_line_item_uuid IN (
        WITH RECURSIVE t AS (
          SELECT report_line_item_uuid FROM report_line_items WHERE report_line_item_uuid = p_moved_item_uuid AND report_structure_uuid = p_structure_uuid
          UNION ALL
          SELECT c.report_line_item_uuid FROM report_line_items c JOIN t ON c.parent_report_line_item_uuid = t.report_line_item_uuid WHERE c.report_structure_uuid = p_structure_uuid
  ...
  -- Leaf/type enforcement around affected nodes
  ...
  WITH counts AS (
    ...
    FROM report_line_items r
    WHERE r.report_structure_uuid = p_structure_uuid AND r.report_line_item_uuid IN (
      SELECT x FROM (VALUES (p_moved_item_uuid), (v_old_parent_uuid), (p_new_parent_uuid)) AS v(x)
    )
  )
  UPDATE report_line_items r
  SET line_item_type = COALESCE(CASE WHEN c.child_count IS NULL THEN r.line_item_type WHEN c.child_count > 0 THEN 'header' ELSE 'leaf' END, r.line_item_type),
      is_calculated = CASE WHEN p_is_calculated_override IS NOT NULL THEN p_is_calculated_override WHEN (CASE WHEN c.child_count > 0 THEN 'header' ELSE 'leaf' END) <> 'leaf' THEN false ELSE r.is_calculated END
  FROM counts c
  WHERE r.report_line_item_uuid = c.report_line_item_uuid AND r.report_structure_uuid = p_structure_uuid;

  IF p_line_item_type_override IS NOT NULL THEN
    UPDATE report_line_items r
    SET line_item_type = CASE WHEN EXISTS (SELECT 1 FROM report_line_items c WHERE c.parent_report_line_item_uuid = r.report_line_item_uuid AND c.report_structure_uuid = p_structure_uuid)
                              THEN CASE WHEN p_line_item_type_override = 'leaf' THEN 'header' ELSE p_line_item_type_override END
                              ELSE p_line_item_type_override END
    WHERE r.report_line_item_uuid = p_moved_item_uuid AND r.report_structure_uuid = p_structure_uuid;
  END IF;

  -- PHASE D: Final global resequencing via hierarchical traversal
  ...
  v_result := jsonb_build_object(
    ...
    'message', 'Move/rename OK. Offset-first strategy avoids unique collisions; final global resequencing applied.'
  );
