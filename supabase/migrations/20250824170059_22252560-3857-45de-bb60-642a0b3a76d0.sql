...
-- Note: Using partial unique indexes instead of constraints with WHERE clauses
...
-- Update the grant_entity_access function to handle entity group access properly
...
BEGIN
  ...
  IF p_entity_uuid IS NOT NULL THEN
    ...
    ON CONFLICT (user_account_uuid, entity_uuid) WHERE (is_active = true AND entity_uuid IS NOT NULL)
    DO UPDATE SET
  ...
  ELSIF p_entity_group_uuid IS NOT NULL THEN
    ...
    ON CONFLICT (user_account_uuid, entity_group_uuid) WHERE (is_active = true AND entity_group_uuid IS NOT NULL)
    DO UPDATE SET
      access_level = EXCLUDED.access_level,
      granted_by_user_uuid = EXCLUDED.granted_by_user_uuid,
      granted_at = now();
