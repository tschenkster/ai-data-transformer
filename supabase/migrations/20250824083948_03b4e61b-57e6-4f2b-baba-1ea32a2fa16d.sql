...
ALTER TYPE app_role ADD VALUE IF NOT EXISTS 'entity_admin';

-- Add integer ID to user_roles table
ALTER TABLE public.user_roles ADD COLUMN IF NOT EXISTS user_role_id INTEGER GENERATED ALWAYS AS IDENTITY;
ALTER TABLE public.user_roles ADD CONSTRAINT IF NOT EXISTS uq_user_roles_id UNIQUE (user_role_id);

-- Phase 2: Add entity context to existing tables

-- Add entity context to report_structures
ALTER TABLE public.report_structures 
ADD COLUMN IF NOT EXISTS entity_uuid UUID,
ADD COLUMN IF NOT EXISTS entity_id INTEGER;

-- Add foreign key constraint for report_structures
ALTER TABLE public.report_structures 
ADD CONSTRAINT IF NOT EXISTS fk_report_structures_entity 
FOREIGN KEY (entity_uuid) REFERENCES entities(entity_uuid);

-- Add entity context to report_line_items
ALTER TABLE public.report_line_items 
ADD COLUMN IF NOT EXISTS entity_uuid UUID,
ADD COLUMN IF NOT EXISTS entity_id INTEGER;

-- Add foreign key constraint for report_line_items
ALTER TABLE public.report_line_items 
ADD CONSTRAINT IF NOT EXISTS fk_report_line_items_entity 
FOREIGN KEY (entity_uuid) REFERENCES entities(entity_uuid);

-- Add entity context to coa_translation_sessions
...
-- Add constraints for coa_translation_sessions
ALTER TABLE public.coa_translation_sessions 
ADD CONSTRAINT IF NOT EXISTS uq_coa_translation_sessions_id UNIQUE (coa_translation_session_id),
ADD CONSTRAINT IF NOT EXISTS fk_coa_translation_sessions_entity 
FOREIGN KEY (entity_uuid) REFERENCES entities(entity_uuid);

-- Add entity context to security_audit_logs
...
-- Add constraints for security_audit_logs
ALTER TABLE public.security_audit_logs 
ADD CONSTRAINT IF NOT EXISTS uq_security_audit_logs_id UNIQUE (security_audit_log_id),
ADD CONSTRAINT IF NOT EXISTS fk_security_audit_logs_entity 
FOREIGN KEY (entity_uuid) REFERENCES entities(entity_uuid);
