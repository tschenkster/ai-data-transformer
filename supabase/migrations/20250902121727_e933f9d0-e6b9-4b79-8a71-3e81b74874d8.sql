-- Fix the source check constraint to allow ai_generated
...
-- Also fix the report_structures_translations table
...
-- Also fix the ui_translations table  
...
-- Create the missing RPC function for saving translations
  ...
  LOOP
    ...
    IF p_entity_type = 'report_structure' THEN
      INSERT INTO report_structures_translations (
        ...
        text_value, -- For now, use the translated text as original since we don't have original
        'ai_generated'
      )
