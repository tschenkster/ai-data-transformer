...
-- Phase 2: Comprehensive backfill of ALL audit fields
DO $$
DECLARE
  system_user_id uuid;
  admin_user_id uuid;
...
BEGIN
  ...
  -- UI translations: Set created_by where NULL
  UPDATE ui_translations 
  SET created_by = system_user_id
  WHERE created_by IS NULL;
  GET DIAGNOSTICS backfill_count = ROW_COUNT;
  RAISE NOTICE 'Backfilled created_by for % UI translation records', backfill_count;
  
  -- UI translations: Set updated_by where NULL  
  UPDATE ui_translations 
  SET updated_by = system_user_id
  WHERE updated_by IS NULL;
  GET DIAGNOSTICS backfill_count = ROW_COUNT;
  RAISE NOTICE 'Backfilled updated_by for % UI translation records', backfill_count;
  
  -- Report structures translations: Set created_by where NULL
  UPDATE report_structures_translations 
  SET created_by = system_user_id
  WHERE created_by IS NULL;
  GET DIAGNOSTICS backfill_count = ROW_COUNT;
  RAISE NOTICE 'Backfilled created_by for % report structure translation records', backfill_count;
  
  -- Report structures translations: Set updated_by where NULL
  UPDATE report_structures_translations 
  SET updated_by = system_user_id
  WHERE updated_by IS NULL;
  GET DIAGNOSTICS backfill_count = ROW_COUNT;
  RAISE NOTICE 'Backfilled updated_by for % report structure translation records', backfill_count;
  
  -- Report line items translations: Set created_by where NULL
  UPDATE report_line_items_translations 
  SET created_by = system_user_id
  WHERE created_by IS NULL;
  GET DIAGNOSTICS backfill_count = ROW_COUNT;
  RAISE NOTICE 'Backfilled created_by for % report line item translation records', backfill_count;
  
  -- Report line items translations: Set updated_by where NULL
  UPDATE report_line_items_translations 
  SET updated_by = system_user_id
  WHERE updated_by IS NULL;
  GET DIAGNOSTICS backfill_count = ROW_COUNT;
  RAISE NOTICE 'Backfilled updated_by for % report line item translation records', backfill_count;
  
  RAISE NOTICE 'Completed comprehensive backfill with system user: %', system_user_id;
END $$;

-- Phase 3: Add NOT NULL constraints after comprehensive backfill
...
-- Phase 4: Replace existing triggers with comprehensive audit triggers
-- UI Translations
