...
-- Create partial unique indexes instead of constraints (PostgreSQL syntax for partial unique constraints)
CREATE UNIQUE INDEX unique_active_entity_access 
  ON user_entity_access (user_uuid, entity_uuid) 
  WHERE is_active = true AND entity_uuid IS NOT NULL;

CREATE UNIQUE INDEX unique_active_entity_group_access 
  ON user_entity_access (user_uuid, entity_group_uuid) 
  WHERE is_active = true AND entity_group_uuid IS NOT NULL;

-- Recreate other indexes with new column names
...
BEGIN
  ...
  IF p_entity_uuid IS NOT NULL THEN
    ...
    IF NOT FOUND THEN
      ...
      ON CONFLICT ON CONSTRAINT unique_active_entity_access
      DO UPDATE SET
  ...
  ELSE
    ...
    IF NOT FOUND THEN
      ...
      ON CONFLICT ON CONSTRAINT unique_active_entity_group_access
      DO UPDATE SET
        access_level = EXCLUDED.access_level,
        granted_by_user_uuid = EXCLUDED.granted_by_user_uuid,
        granted_at = now();
