-- Drop and recreate functions with proper search_path to fix security issues
-- We need to drop them first since we're changing return types

DROP FUNCTION IF EXISTS public.enhanced_check_rate_limit(text,text,integer,integer);
DROP FUNCTION IF EXISTS public.enhanced_log_security_event(text,uuid,jsonb,text,text);

-- Recreate enhanced_check_rate_limit function with proper search path
...
-- Recreate enhanced_log_security_event function with proper search path
